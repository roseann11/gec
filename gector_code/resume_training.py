...
resume_training.py
==================
Use this script to RESUME GECToR training after a Kaggle session crash.

How it works
------------
1. Run this script with the SAME arguments as your original train.py run.
2. It looks for  <model_dir>/checkpoint_resume.pt
3. If found  → it restores model weights, optimizer state, scheduler state,
               epoch counter, best val loss, patience counter, and history,
               then continues training from the next epoch.
4. If NOT found → it starts fresh (same behaviour as train.py).

Usage (Kaggle cell)
-------------------
    !python resume_training.py \
        --train_set  /kaggle/input/gec-data/train.txt \
        --dev_set    /kaggle/input/gec-data/dev.txt \
        --model_dir  /kaggle/working/gector_output \
        --n_epoch    20 \
        --batch_size 32 \
        [... same flags as your original train.py run ...]

Files saved per epoch inside <model_dir>
-----------------------------------------
  checkpoint_resume.pt          ← 🔄 PRIMARY resume file (always latest)
  checkpoint_resume_epochN.pt   ← per-epoch backup (roll back if needed)
  model_best.th                 ← best model weights only
  model_last.th                 ← last epoch weights only
  model_epochN.th               ← per-epoch weights
  model.th                      ← copy of best (for inference / predict.py)
  training_history.json         ← all train/val losses
  metrics.json                  ← final summary

Tips for Kaggle
---------------
* Mount your Kaggle Output or a Dataset as <model_dir> so checkpoints
  persist across sessions.
* After each session ends, the checkpoint is already saved — just re-run
  this cell in the new session.
"""
...
# ↓ Import the UPDATED trainer (with resume support)
...
def load_vocab(vocab_path: str):
    ...
    with open(os.path.join(vocab_path, "labels.txt"), encoding="utf-8") as f:
        for idx, line in enumerate(f):
            token = line.strip()
            if token:
                vocab["labels"][token] = idx

    with open(os.path.join(vocab_path, "d_tags.txt"), encoding="utf-8") as f:
        for idx, line in enumerate(f):
            token = line.strip()
            if token:
    ...
    print(f"Vocab loaded: {len(vocab['labels'])} labels, "
          f"{len(vocab['d_tags'])} d_tags")
    return vocab
...
def get_data_reader(model_name, max_len, skip_correct=False, skip_complex=0,
    ...
    return Seq2LabelsDatasetReader(
        max_len=max_len,
...
def get_model(model_name, vocab, tune_bert=True,
              predictor_dropout=0.0, label_smoothing=0.0,
              confidence=0.0, special_tokens_fix=0):
    model = Seq2Labels(
        encoder_name=model_name,
        num_labels_classes=len(vocab["labels"]),
        num_detect_classes=len(vocab["d_tags"]),
        predictor_dropout=predictor_dropout,
        label_smoothing=label_smoothing,
        confidence=confidence,
        tune_bert=bool(tune_bert),  # convert int to bool
        special_tokens_fix=special_tokens_fix
    )
    model.incorr_index = vocab["d_tags"].get("INCORRECT", 1)
    return model
...
def main(args):
    ...
    import shutil
    vocab_dest = os.path.join(args.model_dir, "vocabulary")
    if not os.path.exists(vocab_dest):
        shutil.copytree(vocab_path, vocab_dest)
    ...
    # ── Pretrain weights (only used when there is NO checkpoint yet) ──
    checkpoint_path = os.path.join(args.model_dir, "checkpoint_resume.pt")
    if not os.path.exists(checkpoint_path):
        if args.pretrain and args.pretrain_folder:
            pretrain_path = os.path.join(
                args.pretrain_folder, f"{args.pretrain}.th"
            )
    ...
    # ── Create trainer — resume=True enables auto-checkpoint loading ──
    trainer = Trainer(
        model=model,
        optimizer=optimizer,
        scheduler=scheduler,
        ...
        shuffle=False,
        resume=True      # ← KEY: always try to resume
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Resume-capable GECToR training script"
    )
